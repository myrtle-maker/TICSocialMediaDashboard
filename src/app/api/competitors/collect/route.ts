...
export async function POST() {
  try {
    ...
    // Find running jobs that belong to competitors (accountId starts with "competitor:")
    const runningJobs = await prisma.scrapeJob.findMany({
      where: {
        status: "running",
        accountId: { startsWith: "competitor:" },
      },
    });

    if (runningJobs.length === 0) {
      return NextResponse.json({ message: "No running competitor jobs to collect", collected: 0, results: [] });
    }
    ...
    for (const job of runningJobs) {
      try {
        ...
        // Extract the competitor ID from "competitor:clxyz..."
        const competitorId = job.accountId.replace("competitor:", "");
        ...
        // Transform using the same pipeline (accountId is just used for the transform, not stored)
        const posts = transformer(items as unknown[], competitorId);
        let upserted = 0;
        ...
        for (const post of posts) {
          try {
            ...
            console.error("Failed to upsert competitor post:", postErr);
          }
        }
        ...
        console.error(`Failed to collect competitor job ${job.id}:`, msg);
    ...
    console.error("Competitor collect endpoint error:", err);
